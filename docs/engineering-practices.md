...
fails whenever `docs/api/openapiv1.json` is not byte-for-byte what the API generates, line endings
excepted (ADR-0053), and
names the first JSON paths that differ. To regenerate, from `backend/`:
