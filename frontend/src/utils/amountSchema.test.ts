...
describe('amountSchema', () => {
  ...
  it('rejects above the available balance when one is given', () => {
    ...
    // No balance provided (deposit) → the balance rule does not apply. The max is the contract's
    // one bound for every money move (MONEY_MAX), not a deposit-specific figure.
