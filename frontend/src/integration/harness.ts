    ...
    if (problem.status === 429) {
      throw new Error(
        'Login was rate-limited (429). The BFF allows 10 auth requests per 60s per IP ' +
          '(RateLimiting:AuthPermitLimit); register, reauthenticate and the azuretag rename share ' +
          'the budget, verify-pin and logout do not. Wait about a minute and re-run.',
      );
    }
