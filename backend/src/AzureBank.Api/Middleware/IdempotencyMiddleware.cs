...
public class IdempotencyMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, IIdempotencyService idempotency)
    {
        ...
        // same bytes). EnableBuffering spools large bodies to disk; the
        // monetary endpoints additionally cap the body via [RequestSizeLimit].
        context.Request.EnableBuffering();
        var requestHash = await idempotency.ComputeRequestHashAsync(
