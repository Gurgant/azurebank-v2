...
public class AuditOptions
{
    ...
    /// does not start a new run of anchors: it stops the chain. Every record names the key that
    /// wrote it, a run holding another cannot check any of them, and <c>anchor</c> refuses to
    /// append — on every run, not once. Measured 2026-09-11 on a scratch database: exit 6 and
    /// "Broke at anchor: 1" twice under the new key, <c>verify</c> still CHAIN INTACT but its
    /// uncovered window "not computed", and <c>anchor</c> appending again (record 3) as soon as the
    /// old key came back. Keep this key for the life of the anchor chain; a ring for it is not
    /// built.
    /// </para>
    /// </remarks>
